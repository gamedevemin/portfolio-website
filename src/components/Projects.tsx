        ...
        {projects.map((project) => (
          <div
            ...
            className={`bg-black rounded-lg overflow-visible shadow-lg hover:shadow-xl transition-all duration-500 cursor-pointer group
              border border-gray-800 hover:border-amber-500/50 relative mt-6
              ${selectedProject === project.id ? 'lg:col-span-3 md:col-span-2 col-span-1 border-amber-500/50' : ''}`}
          >
            {/* Enhanced Under Construction Label */}
            <div className="absolute -top-4 right-4 -rotate-2 px-4 py-2 bg-gradient-to-r from-amber-400/90 via-amber-500/90 to-amber-600/90 text-black text-xs font-semibold rounded-lg shadow-xl border border-amber-400/30 backdrop-blur-sm z-20 transform hover:scale-105 transition-all duration-300 whitespace-nowrap block">
              🚧 Yapım Aşamasında
            </div>
            <div className={`grid ${selectedProject === project.id ? 'lg:grid-cols-2' : 'grid-cols-1'} gap-6`}>
              <div>
                {/* Title Only Initially */}
                <div className="p-6 border-b border-gray-800 group-hover:border-amber-500/30">
                  <h3 className="text-xl font-bold text-white group-hover:text-amber-500 transition-colors mt-4">
                    {project.title}
                  </h3>
                </div>
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.technologies.map((tech, index) => (
                          <span
                            key={index}
                            className="px-2 py-1 text-xs font-medium bg-gray-700 text-gray-300 rounded-full border border-gray-700 hover:border-amber-500/30"
                          >
                            {tech}
                          </span>
                        ))}
              ...
              {selectedProject === project.id && (
                <div className="p-6 space-y-6 border-l border-gray-800">
                  {/* Period */}
                  <div>
                    <h3 className="text-lg font-semibold text-amber-500">Dönem</h3>
                    <p className="text-gray-300">{project.period}</p>
                  </div>

                  {/* Description */}
                  <div>
                    <h3 className="text-lg font-semibold text-amber-500">Proje Detayları</h3>
                  ...
                  {/* Features */}
                  <div>
                    <h3 className="text-lg font-semibold text-amber-500">Özellikler</h3>
                  ...
                  {project.id === 'location-finder' && (
                    <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
                      <h3 className="text-lg font-semibold text-amber-500 mb-4">Nasıl Çalışır?</h3>
                      
                      {/* Step 1 */}
                      <div className="mb-4 p-3 bg-gray-800 rounded border border-gray-700">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="w-6 h-6 rounded-full bg-amber-500 text-black flex items-center justify-center text-sm font-bold">1</span>
                      ...
                      <div className="mb-4 p-3 bg-gray-800 rounded border border-gray-700">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="w-6 h-6 rounded-full bg-amber-500 text-black flex items-center justify-center text-sm font-bold">2</span>
                      ...
                      <div className="p-3 bg-gray-800 rounded border border-gray-700">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="w-6 h-6 rounded-full bg-amber-500 text-black flex items-center justify-center text-sm font-bold">3</span>
                          <span className="text-gray-300 font-medium">Sonuçlar</span>
                        </div>
                        <div className="space-y-2">
                          <div className="p-2 bg-gray-700/50 rounded text-sm text-gray-400">
                            <div className="font-medium text-amber-500">Espresso Lab</div>
                            <div className="text-xs text-gray-500">Zeyrek, Fatih</div>
                          </div>
                          <div className="p-2 bg-gray-700/50 rounded text-sm text-gray-400">
                            <div className="font-medium text-amber-500">Espresso Lab Premium</div>
